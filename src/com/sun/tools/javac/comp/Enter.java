...
/** 
 * 此类将所有类中出现的符号输入到符号表中。而这个过程是由以下两个阶段组成的:
 * This class enters symbols for all encountered definitions into
 *  the symbol table. The pass consists of two phases, organized as
 *  follows:
 *
 *	第一阶段，所有类符号中出现在符号输入到自己的符号表中，并递归其它类的符号,
 *   类符号是由MemberEnter对象来处理的。
 ...
 *  
 *  
 *  第二阶段，调用MemberEnter.complete()方法，
 *  
 *  Enter.uncompleted包括所有类符号、类的参数类型符号也就是泛型、父类符号、接口类型符
 *  
 *  MemberEnter.halfcompleted基于Enter.uncompleted的。将类的参数类型符号也就是泛型、父类符号、接口类型符
 *  存储到一个未处理列表(Enter.uncompleted)中,并将这个未处理列表中的所有符号都解析到各自的类符号表中，而类符号则是在第一阶段进行输入的。
 *  
    ...
    public void complete(List<JCCompilationUnit> trees, ClassSymbol c) {
        ...
        try {
            ...
            if  (memberEnter.completionEnabled) {
                ...
                /**
                 * uncompleted列表没有的符号(除类符号外)，根据improt声明，给每一个类的符号都添加了一个MemberEnter对象
                 * 并将这些符号(包括类的参数类型符号也就是泛型、父类符号、接口类型符等)放入uncompleted列表
                 */
